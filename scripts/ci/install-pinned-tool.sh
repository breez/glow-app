...
#
# Replaces the `curl … | sh` installers the SDK warmers used to run, so the
# pin is the artifact rather than whatever a URL serves today.
#
# Every executable in the archive lands in ~/.cargo/bin: these tools ship
# helper binaries alongside the main one and expect them on PATH.
set -euo pipefail
